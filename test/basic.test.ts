...
describe("Basic tests", () => {
  ...
  describe("When executed in the context of root directory", () => {
    it("should dereference successfully", async () => {
      const schema = await dereference(
        "specs/absolute-root/absolute-root.yaml",
        resolver,
      )
  ...
  describe("$refs that are substrings of each other", () => {
    it("should dereference successfully", async () => {
      const schema = await dereference(
        "specs/substrings/substrings.yaml",
        resolver,
      )
      expect(schema).toMatchObject(substrings.dereferenced)
      // Reference equality
      expect(schema.properties.firstName).toEqual(schema.definitions.name)
      expect(schema.properties.middleName).toEqual(
        schema.definitions["name-with-min-length"],
      )
      expect(schema.properties.lastName).toEqual(
        schema.definitions["name-with-min-length-max-length"],
      )
    })
