...
describe("AsyncAPI v3 tests", () => {
  describe("AsyncAPI v3 with channel-scoped message references", () => {
    it("should bundle successfully and preserve channel-scoped message refs", async () => {
      const schema: any = await bundle(
        "specs/asyncapi-v3/asyncapi-v3.yaml",
        resolver,
      )
      expect(schema).toMatchObject(asyncApiV3.bundled)
      expect(schema.operations.SendText.messages[0].$ref).toBe(
        "#/channels/SpeakChannel/messages/TextInput",
      )
      expect(schema.operations.ReceiveAudio.messages[0].$ref).toBe(
        "#/channels/SpeakChannel/messages/AudioOutput",
      )
    })

    it("should dereference successfully", async () => {
      const schema: any = await dereference(
        "specs/asyncapi-v3/asyncapi-v3.yaml",
        resolver,
      )
      expect(schema).toMatchObject(asyncApiV3.dereferenced)
      expect(schema.operations.SendText.messages[0]).toHaveProperty("payload")
      expect(schema.operations.ReceiveAudio.messages[0]).toHaveProperty(
        "payload",
      )
    })
  })
  ...
  describe("AsyncAPI v3 with external file references", () => {
    it("should bundle external refs successfully", async () => {
      const schema: any = await bundle(
        "specs/asyncapi-v3-external/asyncapi-v3-external.yaml",
        resolver,
      )
      ...
      expect(schema.channels.SpeakChannel.messages.TextInput.$ref).toBe(
        "#/components/messages/GenericTextMessage",
      )
      // Verify operation channel refs are internal
      expect(schema.operations.SendText.channel.$ref).toBe(
        "#/channels/SpeakChannel",
      )
    ...
    it("should dereference external refs successfully", async () => {
      ...
      expect(schema.components.messages.GenericTextMessage.payload.type).toBe(
        "object",
      )
      expect(schema.channels.SpeakChannel.messages.TextInput.payload.type).toBe(
        "object",
      )
  ...
  describe("AsyncAPI v3 comprehensive - all component types", () => {
    it("should bundle comprehensive spec with all component types", async () => {
      const schema: any = await bundle(
        "specs/asyncapi-v3-comprehensive/asyncapi-v3-comprehensive.yaml",
        resolver,
      )
      expect(schema).toMatchObject(asyncApiV3Comprehensive.bundled)

      // Verify server refs are preserved
      expect(schema.servers.production.variables.version.$ref).toBe(
        "#/components/serverVariables/apiVersion",
      )
      expect(schema.servers.production.security[0].$ref).toBe(
        "#/components/securitySchemes/apiKey",
      )
      expect(schema.servers.production.tags[0].$ref).toBe(
        "#/components/tags/production",
      )
      expect(schema.servers.production.externalDocs.$ref).toBe(
        "#/components/externalDocs/serverDocs",
      )

      // Verify channel refs are preserved
      expect(schema.channels.UserChannel.servers[0].$ref).toBe(
        "#/servers/production",
      )
      expect(schema.channels.UserChannel.parameters.userId.$ref).toBe(
        "#/components/parameters/userId",
      )
      expect(
        schema.channels.UserChannel.messages.UserCreated.headers.$ref,
      ).toBe("#/components/schemas/MessageHeaders")
      expect(
        schema.channels.UserChannel.messages.UserCreated.payload.$ref,
      ).toBe("#/components/schemas/User")
      expect(
        schema.channels.UserChannel.messages.UserCreated.correlationId.$ref,
      ).toBe("#/components/correlationIds/default")
      expect(
        schema.channels.UserChannel.messages.UserCreated.traits[0].$ref,
      ).toBe("#/components/messageTraits/commonHeaders")
      expect(schema.channels.UserChannel.tags[0].$ref).toBe(
        "#/components/tags/users",
      )
      expect(schema.channels.UserChannel.externalDocs.$ref).toBe(
        "#/components/externalDocs/channelDocs",
      )

      // Verify operation refs are preserved
      expect(schema.operations.CreateUser.channel.$ref).toBe(
        "#/channels/UserChannel",
      )
      expect(schema.operations.CreateUser.messages[0].$ref).toBe(
        "#/channels/UserChannel/messages/UserCreated",
      )
      expect(schema.operations.CreateUser.security[0].$ref).toBe(
        "#/components/securitySchemes/oauth2",
      )
      expect(schema.operations.CreateUser.traits[0].$ref).toBe(
        "#/components/operationTraits/commonOp",
      )
      expect(schema.operations.CreateUser.tags[0].$ref).toBe(
        "#/components/tags/users",
      )
      expect(schema.operations.CreateUser.externalDocs.$ref).toBe(
        "#/components/externalDocs/operationDocs",
      )

      // Verify reply refs are preserved
      expect(schema.operations.CreateUser.reply.address.$ref).toBe(
        "#/components/replyAddresses/default",
      )
      expect(schema.operations.CreateUser.reply.channel.$ref).toBe(
        "#/channels/UserChannel",
      )
      expect(schema.operations.CreateUser.reply.messages[0].$ref).toBe(
        "#/channels/UserChannel/messages/UserUpdated",
      )

      // Verify component refs are preserved
      expect(schema.components.schemas.User.properties.profile.$ref).toBe(
        "#/components/schemas/Profile",
      )
      expect(schema.components.messageTraits.commonHeaders.headers.$ref).toBe(
        "#/components/schemas/MessageHeaders",
      )
      expect(
        schema.components.messageTraits.commonHeaders.correlationId.$ref,
      ).toBe("#/components/correlationIds/default")
      expect(schema.components.operationTraits.commonOp.tags[0].$ref).toBe(
        "#/components/tags/common",
      )
      expect(schema.components.tags.users.externalDocs.$ref).toBe(
        "#/components/externalDocs/tagDocs",
      )
    })
    ...
    it("should dereference comprehensive spec with all component types", async () => {
      ...
      expect(schema.servers.production.externalDocs.url).toBe(
        "https://docs.example.com/servers",
      )

      // Verify channel refs are dereferenced
      expect(schema.channels.UserChannel.servers[0].host).toBe(
        "api.example.com",
      )
      expect(schema.channels.UserChannel.parameters.userId.description).toBe(
        "User identifier",
      )
      expect(
        schema.channels.UserChannel.messages.UserCreated.headers.type,
      ).toBe("object")
      expect(
        schema.channels.UserChannel.messages.UserCreated.payload.type,
      ).toBe("object")
      expect(
        schema.channels.UserChannel.messages.UserCreated.correlationId.location,
      ).toBe("$message.header#/correlationId")
      expect(
        schema.channels.UserChannel.messages.UserCreated.traits[0].headers.type,
      ).toBe("object")

      // Verify operation refs are dereferenced
      expect(schema.operations.CreateUser.channel.address).toBe(
        "/users/{userId}",
      )
      expect(schema.operations.CreateUser.messages[0].payload.type).toBe(
        "object",
      )
      expect(schema.operations.CreateUser.security[0].type).toBe("oauth2")
      expect(schema.operations.CreateUser.traits[0].description).toBe(
        "Common operation trait",
      )
      expect(schema.operations.CreateUser.tags[0].name).toBe("users")
      expect(schema.operations.CreateUser.externalDocs.url).toBe(
        "https://docs.example.com/operations",
      )

      // Verify reply refs are dereferenced
      expect(schema.operations.CreateUser.reply.address.location).toBe(
        "$message.header#/replyTo",
      )
      expect(schema.operations.CreateUser.reply.channel.address).toBe(
        "/users/{userId}",
      )
      expect(schema.operations.CreateUser.reply.messages[0].payload.type).toBe(
        "object",
      )

      // Verify nested schema refs are dereferenced
      expect(schema.components.schemas.User.properties.profile.type).toBe(
        "object",
      )
    })
  })
  ...
  describe("AsyncAPI v3 with circular schema references", () => {
    it("should bundle with circular schemas", async () => {
      const schema: any = await bundle(
        "specs/asyncapi-v3-circular/circular.yaml",
        resolver,
      )
      expect(schema).toMatchObject(asyncApiV3Circular.bundled)

      // Verify circular refs are preserved as $refs in bundle
      expect(schema.components.schemas.User.properties.friend.$ref).toBe(
        "#/components/schemas/User",
      )
      expect(schema.components.schemas.Order.properties.customer.$ref).toBe(
        "#/components/schemas/User",
      )
      expect(schema.components.schemas.Review.properties.reviewer.$ref).toBe(
        "#/components/schemas/User",
      )
    })
    ...
    it("should dereference with enableCircular: false (preserve circular $refs)", async () => {
      ...
      const circularRefPath =
        "#/channels/UserChannel/messages/UserMessage/payload"

      // Direct self-reference: User.friend should have $ref
      expect(
        schema.channels.UserChannel.messages.UserMessage.payload.properties
          .friend.$ref,
      ).toBe(circularRefPath)

      // Verify Order schema is dereferenced with circular ref to User
      expect(schema.components.schemas.Order.type).toBe("object")
      expect(schema.components.schemas.Order.properties.orderId.type).toBe(
        "string",
      )
      ...
      expect(
        schema.channels.UserChannel.messages.UserMessage.payload.properties
          .orders.items.properties.orderId.type,
      ).toBe("string")
    ...
    it("should dereference with enableCircular: true (actual circular object references)", async () => {
      ...
      const canonicalUser =
        schema.channels.UserChannel.messages.UserMessage.payload
      ...
      expect(schema.components.schemas.User.properties.friend).toBe(
        canonicalUser,
      )
  ...
  describe("AsyncAPI v3 edge cases", () => {
    describe("Sibling content with $ref (EDG-SIB-*)", () => {
      it("should preserve sibling properties alongside $ref during bundle", async () => {
        const schema: any = await bundle(
          "specs/asyncapi-v3-edge/sibling-content.yaml",
          resolver,
        )
        expect(schema).toMatchObject(asyncApiV3Edge.sibling.bundled)

        // EDG-SIB-001: $ref with description sibling
        expect(schema.channels.UserChannel.messages.UserCreated.$ref).toBe(
          "#/components/messages/BaseUserMessage",
        )
        expect(
          schema.channels.UserChannel.messages.UserCreated.description,
        ).toBe("User creation event with additional context")

        // EDG-SIB-002: $ref with multiple sibling properties
        expect(schema.channels.UserChannel.messages.UserUpdated.$ref).toBe(
          "#/components/messages/BaseUserMessage",
        )
        expect(
          schema.channels.UserChannel.messages.UserUpdated.description,
        ).toBe("User update event")
        expect(schema.channels.UserChannel.messages.UserUpdated.title).toBe(
          "UserUpdatedEvent",
        )

        // EDG-SIB-002: Schema $ref with sibling properties
        expect(schema.components.schemas.ExtendedUser.$ref).toBe(
          "#/components/schemas/BaseUser",
        )
        expect(schema.components.schemas.ExtendedUser.description).toBe(
          "Extended user with additional metadata",
        )
        expect(schema.components.schemas.ExtendedUser.title).toBe(
          "ExtendedUserSchema",
        )
      })
    ...
    describe("Name collision handling (BND-COL-*)", () => {
      it("should handle name collisions by adding numeric suffix", async () => {
        const schema: any = await bundle(
          "specs/asyncapi-v3-edge/name-collision/main.yaml",
          resolver,
        )
        ...
        expect(
          schema.channels.OrderChannel.messages.OrderMessage.payload.properties
            .customerFromFileA.$ref,
        ).toBe("#/components/schemas/User")
        expect(schema.components.schemas.User.description).toBe(
          "User from file A (customer)",
        )
        ...
        expect(schema.components.schemas.User1.description).toBe(
          "User from file B (seller)",
        )
        ...
        expect(
          schema.components.schemas.Product.properties.internalOnly,
        ).toBeDefined()
        expect(
          schema.components.schemas.Product1.properties.externalOnly,
        ).toBeDefined()
      })
    })
    ...
    describe("Error handling (EDG-ERR-*)", () => {
      it("should preserve invalid refs during bundle without error", async () => {
        const errors: string[] = []
        const schema: any = await bundle(
          "specs/asyncapi-v3-edge/error-cases.yaml",
          resolver,
          {
            hooks: {
        ...
        expect(
          schema.channels.ValidChannel.messages.InvalidMessage.payload.$ref,
        ).toBe("#/components/schemas/NonExistentSchema")
        expect(schema.operations.InvalidOperation.channel.$ref).toBe(
  ...
  describe("AsyncAPI v3 with external traits and bindings", () => {
    it("should bundle external traits and bindings into components", async () => {
      const schema: any = await bundle(
        "specs/asyncapi-v3-traits-bindings/main.yaml",
        resolver,
      )
      expect(schema).toMatchObject(asyncApiV3TraitsBindings.bundled)

      // Verify external operationTraits are bundled
      expect(schema.operations.SendUser.traits[0].$ref).toBe(
        "#/components/operationTraits/commonOp",
      )
      expect(schema.components.operationTraits.commonOp.description).toBe(
        "Common operation trait from external file",
      )

      // Verify external messageTraits are bundled
      expect(
        schema.channels.UserChannel.messages.UserMessage.traits[0].$ref,
      ).toBe("#/components/messageTraits/commonHeaders")
      expect(schema.components.messageTraits.commonHeaders.contentType).toBe(
        "application/json",
      )

      // Verify external serverBindings are bundled
      expect(schema.servers.production.bindings.$ref).toBe(
        "#/components/serverBindings/websocket",
      )
      expect(schema.components.serverBindings.websocket.ws.method).toBe("GET")

      // Verify external channelBindings are bundled
      expect(schema.channels.UserChannel.bindings.$ref).toBe(
        "#/components/channelBindings/websocket",
      )
      expect(schema.components.channelBindings.websocket.ws.method).toBe("GET")

      // Verify external operationBindings are bundled
      expect(schema.operations.SendUser.bindings.$ref).toBe(
        "#/components/operationBindings/kafka",
      )
      expect(schema.components.operationBindings.kafka.kafka.groupId.type).toBe(
        "string",
      )

      // Verify external messageBindings are bundled
      expect(
        schema.channels.UserChannel.messages.UserMessage.bindings.$ref,
      ).toBe("#/components/messageBindings/kafka")
      expect(
        schema.components.messageBindings.kafka.kafka.schemaIdLocation,
      ).toBe("header")

      // Verify local traits are preserved
      expect(schema.components.operationTraits.localTrait.description).toBe(
        "A local operation trait",
      )
      expect(
        schema.components.messageTraits.localMessageTrait.contentType,
    ...
    it("should dereference external traits and bindings", async () => {
      ...
      expect(schema.operations.SendUser.traits[0].description).toBe(
        "Common operation trait from external file",
      )

      // Verify messageTraits are dereferenced
      expect(
        schema.channels.UserChannel.messages.UserMessage.traits[0].contentType,
      ).toBe("application/json")
      expect(
        schema.channels.UserChannel.messages.UserMessage.traits[0].headers.type,
      ).toBe("object")
      ...
      expect(schema.operations.SendUser.bindings.kafka.groupId.type).toBe(
        "string",
      )

      // Verify messageBindings are dereferenced
      expect(
        schema.channels.UserChannel.messages.UserMessage.bindings.kafka
          .schemaIdLocation,
      ).toBe("header")
    })
  })
